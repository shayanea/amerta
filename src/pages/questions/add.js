...
class addQuestion extends Component {
  ...
  getCategory = () => {
    return axios
      .get("/questioncategories?StatusId=1")
      .then(res => this.setState({ cateories: res.data.data, hasCategoryLoaded: true }))
      .catch(err => this.setState({ hasCategoryLoaded: true }));
  };
  ...
  render() {
    ...
    const { isLoading, hasCategoryLoaded, cateories } = this.state;
    return (
        ...
        <Row>
          <Form disableEnterSubmit={false} className="add-form" horizontal onSubmit={handleSubmit(this.submit)}>
            <FormSelectField
              ...
              optionText="title"
            />
            <FormInputField name="body" type="textarea" autoSize label="Body" placeholder="Body" />
            <FormSelectField
              autoWidth
